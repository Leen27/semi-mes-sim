...
## ADR-011：引入干净状态检查与定期清理机制

- **状态**: 已接受
- **日期**: 2026-06-07
- **背景**: Agent 会话结束时经常留下临时文件、未更新的进度文档、破损的构建状态。新会话需要 30-60 分钟才能搞清楚上一轮实际做了什么。干净状态是会话完成的必要条件，不是可选的家务。
- **决策**:
  1. 创建 `scripts/session-exit-check.sh` — 五维度干净状态检查脚本（构建/测试/进度/产物/启动）
  2. 创建 `scripts/session-cleanup.sh` — 幂等清理脚本，删除临时文件、扫描 debug 代码、清理空目录
  3. 创建 `docs/quality.md` — 模块质量追踪文档，持续记录各包的健康度
  4. 将「干净状态检查」嵌入 Clock Out 流程，与 `make check` 同等重要
  5. 建立双模式清理策略：即时清理（每次会话）+ 定期清理（每月第一周）
  6. 建立 Harness 简化机制：每月审视一个组件，验证是否可被模型自主替代
- **拒绝的替代方案**:
  - 依赖 agent 自律清理（"稍后清理"等于永不清理，entropy 的默认状态是增长）
  - 每周五集中清理（OpenAI 经验：占 20% 时间，不可扩展；小额持续清理优于大额集中清理）
- **后果**:
  - 每次会话结束时多花 2-3 分钟执行 cleanup + exit-check
  - 新会话启动时间从潜在的 30-60 分钟压缩到 3 分钟以内
  - 质量文档使代码库健康度可追踪，防止「破窗效应」
  - 定期 Harness 简化确保约束不过度膨胀

## ADR-010：引入运行时与过程可观测性

- **状态**: 已接受
- **日期**: 2026-06-07
- **背景**: Agent 执行任务时缺乏运行时可见性，导致「正确」与「看起来正确」无法区分、评估变成主观判断、跨会话交接时需要 30-50% 的冗余诊断时间。可观测性是 harness 的架构属性，必须从设计之初就内置。
- **决策**:
