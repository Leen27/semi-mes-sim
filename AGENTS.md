...
32. **"稍后清理"等于永不清理** — 每次会话结束必须运行 `scripts/session-cleanup.sh` 进行幂等清理
33. **禁止遗留临时产物** — 包括但不限于：*.tmp、*.log、debug console 语句、未跟踪的空目录
34. **质量文档必须持续更新** — `docs/quality.md` 每次功能达到 passing 时更新对应模块评级；每月第一周全量审查
35. **定期简化 Harness** — 每月审视一次约束和工具链，移除模型能力已能自主完成的冗余约束
